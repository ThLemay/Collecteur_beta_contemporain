        ...
        if (containerInfo.id == camera.s_waitingOrderArr[camera.s_waitingIndex]) and (containerInfo.error == containerErrorCode.VALID):
            ...
            if (time.time() - waiting_process_id.detectionTime) >= WAITING_VALIDATION_DELAY_S:
                ...
                if MAIN_ENABLE_MOTOR:
                    if containerInfo.id in [1, 2]:
                        print(f"Contenant ID {containerInfo.id} → gauche")
                        fork.dumpLeftAndReturnCenter()
                    elif containerInfo.id in [3, 4]:
                        print(f"Contenant ID {containerInfo.id} → droite")
                        fork.dumpRightAndReturnCenter()
                    else:
                        print(f"❓ ID {containerInfo.id} non reconnu → aucun mouvement")
                        sleep(1)
                else:
            ...
            print(f"Wrong input container ! n°{containerInfo.id} instead of n°{camera.s_waitingOrderArr[camera.s_waitingIndex]}")
...
# 🆕 Juste après être sorti du if → c’est qu’on vient de cliquer
            if MAIN_ENABLE_MOTOR:
                print("🔓 Réouverture de la trappe après clic")
                trapdoor.open()
